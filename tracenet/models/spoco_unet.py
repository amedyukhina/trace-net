...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding, is3d):
    ...
    Args:
        ...
        is3d (bool): if True use 3d specific modules, otherwise use 2d
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            assert num_channels % num_groups == 0, \
                f'Expected number of channels in input to be divisible by num_groups. num_channels={num_channels}, ' \
...
class Encoder(nn.Module):
    ...
    Args:
        ...
        num_groups (int): number of groups for the GroupNorm
        padding (int or tuple): add zero-padding added to all three sides of the input
        is3d (bool): use 3d or 2d convolutions
    """
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, basic_module=DoubleConv,
                 conv_layer_order='gcr', num_groups=8, mode='nearest', padding=1, is3d=True):
        super(Decoder, self).__init__()

        # always use interpolation upsampling
        ...
        if basic_module == DoubleConv:
            ...
            # use sum joining for ResUNet
            self.joining = partial(self._joining, concat=False)
            # 1x1 convolution to reduce the number of channels for summation joining; don't use padding in 1x1 conv
            self.conv1x1 = SingleConv(2 * in_channels, in_channels, kernel_size=1, order='gc', num_groups=8, padding=0,
                                      is3d=is3d)
